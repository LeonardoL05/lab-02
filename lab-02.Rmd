...
author: "Team Name: Leonardo Maffei Mercalli, Aksel Huillard, Ben Calder and User4"
date: "`r Sys.Date()`"
...
plastic_data_all$total_mismanage_plastic <- plastic_data_all$mismanaged_plastic * plastic_data_all$population

plastic_data_all %>% head(n = 10)

mean_totals <- plastic_data_all %>% group_by(region) %>% summarise(mean_total = mean(total_mismanage_plastic))
...
plastic_data_all$pct_mismanaged_plastic_ocean <- (plastic_data_all$mismanaged_plastic_ocean / plastic_data_all$total_mismanage_plastic) * 100

median_pct <- plastic_data_all %>% group_by(region) %>% summarise(med_pct = median(pct_mismanaged_plastic_ocean)) %>% drop_na()
